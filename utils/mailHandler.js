...
async function sendPasswordResetMail({
  ...
  const transporter = getTransporter();
  if (!transporter || !to || !resetUrl) return { skipped: true };

  const from = process.env.MAIL_FROM || process.env.MAIL_USER;
  const subject = `[${shopName}] Đặt lại mật khẩu tài khoản`;
  const safeName = escapeHtml(customerName || 'bạn');
  ...
  const html = `
  <div style="background:#eef2f7;padding:28px 12px;font-family:Arial,sans-serif;color:#111827;">
    <div style="max-width:680px;margin:0 auto;background:#ffffff;border-radius:22px;overflow:hidden;border:1px solid #dbe4f0;box-shadow:0 16px 40px rgba(15,23,42,0.08);">
      <div style="background:linear-gradient(135deg, ${brandDark}, ${brandColor});padding:26px 28px;color:#ffffff;">
        <div style="display:inline-block;padding:10px 16px;border-radius:999px;background:rgba(255,255,255,0.12);color:#fff;font-size:20px;font-weight:800;letter-spacing:0.5px;">${safeShopName}</div>
        <div style="margin-top:18px;font-size:26px;font-weight:800;line-height:1.3;">Yêu cầu đặt lại mật khẩu</div>
        <div style="margin-top:8px;font-size:15px;line-height:1.6;opacity:0.95;">Chúng tôi đã nhận được yêu cầu đổi mật khẩu cho tài khoản của bạn.</div>
      </div>
      <div style="padding:28px 24px 30px;">
        <div style="font-size:24px;font-weight:800;color:#0f172a;margin-bottom:10px;">Chào ${safeName},</div>
        <div style="font-size:15px;line-height:1.8;color:#475569;margin-bottom:18px;">Nhấn vào nút bên dưới để đặt lại mật khẩu. Liên kết này sẽ hết hạn sau 10 phút.</div>
        <div style="margin:24px 0;">
          <a href="${safeResetUrl}" style="display:inline-block;padding:14px 22px;border-radius:14px;background:${brandColor};color:#ffffff;text-decoration:none;font-weight:800;">Đổi mật khẩu ngay</a>
        </div>
        <div style="font-size:14px;line-height:1.7;color:#64748b;margin-bottom:12px;">Nếu nút không hoạt động, bạn có thể sao chép liên kết sau và dán vào trình duyệt:</div>
        <div style="word-break:break-all;font-size:14px;line-height:1.7;color:${brandColor};">${safeResetUrl}</div>
        <div style="margin-top:18px;font-size:14px;line-height:1.7;color:#64748b;">Nếu bạn không yêu cầu đổi mật khẩu, hãy bỏ qua email này.</div>
      </div>
    </div>
  </div>`;
  ...
  const text = [
    `Chao ${customerName || 'ban'},`,
    `Ban vua yeu cau dat lai mat khau tai ${shopName}.`,
    'Hay mo lien ket sau de doi mat khau (hieu luc trong 10 phut):',
    resetUrl,
    '',
