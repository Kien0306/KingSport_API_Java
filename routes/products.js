...
function buildInitialSizeStocks(sizes = [], totalStock = 0) {
  const normalizedSizes = normalizeSizes(sizes);
  ...
  const remain = stock % normalizedSizes.length;
  return normalizedSizes.map((size, index) => ({ size, stock: base + (index < remain ? 1 : 0), soldCount: 0 }));
}
